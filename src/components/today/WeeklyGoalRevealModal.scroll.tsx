...
export function WeeklyGoalRevealModal({
  ...
  const followScroll = useCallback(() => {
    if (!autoFollowScrollRef.current) return
  ...
  useEffect(() => {
    ...
    scrollToTop()
  }, [summaries, untargetedStats, scrollToTop])
  ...
  }, [showGoals, scrollToTop])

  useEffect(() => {
    if (!showStats && !showFinale) return
    autoFollowScrollRef.current = true
    followScroll()
  }, [showStats, showFinale, followScroll])
  ...
  }, [showGoals, showFinale, orderedGoals.length, showStats, bodyMaxPx, followScroll])
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              {showStats &&
                untargetedStats.map((stat) => (
                  <StatRevealCard key={stat.id} stat={stat} compact onFollowScroll={followScroll} />
                ))}
            </div>
